...
import org.pineconealliance.blog.dto.*;
...
import org.pineconealliance.blog.enums.FileExtEnum;
import org.pineconealliance.blog.enums.FilePathEnum;
import org.pineconealliance.blog.exception.BizException;
import org.pineconealliance.blog.service.*;
import org.pineconealliance.blog.strategy.context.SearchStrategyContext;
import org.pineconealliance.blog.strategy.context.UploadStrategyContext;
...
import org.pineconealliance.blog.vo.*;
...
public class ArticleServiceImpl extends ServiceImpl<ArticleDao, Article> implements ArticleService {
    ...
    public ArticleDTO getArticleById(Integer articleId) {
        ...
        CompletableFuture<List<ArticleRecommendDTO>> newestArticleList = CompletableFuture.supplyAsync(() -> {
            List<Article> articleList = articleDao.selectList(new LambdaQueryWrapper<Article>()
                    .select(Article::getId, Article::getArticleTitle, Article::getArticleCover,
                            Article::getCreateTime).eq(Article::getIsDelete, CommonConst.FALSE)
                    .eq(Article::getStatus, ArticleStatusEnum.PUBLIC.getStatus()).orderByDesc(Article::getId).last(
                            "limit 5"));
        ...
        Article lastArticle = articleDao.selectOne(new LambdaQueryWrapper<Article>()
                .select(Article::getId, Article::getArticleTitle, Article::getArticleCover).eq(Article::getIsDelete,
                        CommonConst.FALSE)
                .eq(Article::getStatus, ArticleStatusEnum.PUBLIC.getStatus())
    ...
    public void saveOrUpdateArticle(ArticleVO articleVO) {
        // 查询博客配置信息
        CompletableFuture<WebsiteConfigVO> webConfig =
                CompletableFuture.supplyAsync(() -> blogInfoService.getWebsiteConfig());
        // 保存文章分类
    ...
    private Category saveArticleCategory(ArticleVO articleVO) {
        // 判断分类是否存在
        Category category = categoryDao.selectOne(new LambdaQueryWrapper<Category>().eq(Category::getCategoryName,
                articleVO.getCategoryName()));
    ...
    public List<String> exportArticles(List<Integer> articleIdList) {
        ...
        for (Article article : articleList) {
            try (ByteArrayInputStream inputStream = new ByteArrayInputStream(article.getArticleContent().getBytes())) {
                String url =
                        uploadStrategyContext.executeUploadStrategy(article.getArticleTitle() + FileExtEnum.MD.getExtName(), inputStream, FilePathEnum.MD.getPath());
                urlList.add(url);
            } catch (Exception e) {
    ...
    public void updateArticleViewsCount(Integer articleId) {
        ...
        Set<Integer> articleSet =
                CommonUtils.castSet(Optional.ofNullable(session.getAttribute(CommonConst.ARTICLE_SET)).orElseGet(HashSet::new), Integer.class);
    ...
    private void saveArticleTag(ArticleVO articleVO, Integer articleId) {
        ...
        if (CollectionUtils.isNotEmpty(tagNameList)) {
            ...
            if (CollectionUtils.isNotEmpty(tagNameList)) {
                List<Tag> tagList =
                        tagNameList.stream().map(item -> Tag.builder().tagName(item).build()).collect(Collectors.toList());
