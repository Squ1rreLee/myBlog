...
import org.pineconealliance.blog.dao.CommentDao;
import org.pineconealliance.blog.dao.TalkDao;
import org.pineconealliance.blog.dto.CommentCountDTO;
...
import org.pineconealliance.blog.enums.TalkStatusEnum;
import org.pineconealliance.blog.exception.BizException;
import org.pineconealliance.blog.service.RedisService;
import org.pineconealliance.blog.service.TalkService;
import org.pineconealliance.blog.util.*;
...
public class TalkServiceImpl extends ServiceImpl<TalkDao, Talk> implements TalkService {
    ...
    public List<String> listHomeTalks() {
        ...
        return talkDao.selectList(new LambdaQueryWrapper<Talk>()
                        ...
                        .last("limit 10"))
                .stream()
                .map(item -> item.getContent().length() > 200 ? HTMLUtils.deleteHMTLTag(item.getContent().substring(0
                        , 200)) : HTMLUtils.deleteHMTLTag(item.getContent()))
    ...
    public PageResult<TalkBackDTO> listBackTalks(ConditionVO conditionVO) {
        ...
        // 分页查询说说
        List<TalkBackDTO> talkDTOList = talkDao.listBackTalks(PageUtils.getLimitCurrent(), PageUtils.getSize(),
                conditionVO);
        talkDTOList.forEach(item -> {
            // 转换图片格式
            if (Objects.nonNull(item.getImages())) {
    ...
    public TalkBackDTO getBackTalkById(Integer talkId) {
        ...
        if (Objects.nonNull(talkBackDTO.getImages())) {
            talkBackDTO.setImgList(CommonUtils.castList(JSON.parseObject(talkBackDTO.getImages(), List.class),
                    String.class));
        }
        return talkBackDTO;
    }
